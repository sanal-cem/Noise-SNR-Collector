 ...
 */
public class wMapsActivity extends AppCompatActivity
        implements
    ...
    private void addMarkers() {
        wSnrData = new SNRDataPoint();
        wSnrData.fetchSNRDataDb();

        int rNum = wSnrData.routeNum[0];
        for (int i = 0; i < wSnrData.jsonArrLength; i++) {

            if(wSnrData.routeNum[i] != rNum)
                rNum = wSnrData.routeNum[i];

            LatLng point = new LatLng(wSnrData.loc_x[i], wSnrData.loc_y[i]);
            mMap.addMarker(new MarkerOptions()
                    .position(point)
                    .title(String.valueOf(wSnrData.SNR_all[i])));
        }
    }
